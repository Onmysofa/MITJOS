...
sys_page_alloc(envid_t envid, void *va, int perm)
{
    ...
    if((p = page_alloc(ALLOC_ZERO)) == NULL)
        return -E_NO_MEM;
    if(page_insert(e->env_pgdir, p, va, perm|PTE_U|PTE_P) < 0) {
        page_free(p);
        return -E_NO_MEM;
    }
    return 0;
	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
    ...
    if(((uintptr_t)srcva) >= UTOP || ((uintptr_t)srcva) % PGSIZE != 0
        ...
        return -E_INVAL;
    srcpte = pgdir_walk(srce->env_pgdir, srcva, 0);
    ...
    if((perm & PTE_W) && !(*srcpte & PTE_W))
        return -E_INVAL;
    if(page_insert(dste->env_pgdir, pa2page(PTE_ADDR(*srcpte)), dstva, perm|PTE_U|PTE_P) < 0)
        return -E_NO_MEM;
    return 0;
}
...
sys_ipc_try_send(envid_t envid, uint32_t value, void *srcva, unsigned perm)
{
    ...
    if((uint32_t)srcva < UTOP && (uint32_t)e->env_ipc_dstva < UTOP) {
        ...
        if(!(*pte & PTE_W) && (perm & PTE_W))
            return -E_INVAL;
        if(page_insert(e->env_pgdir, pp, e->env_ipc_dstva, perm | PTE_P | PTE_U) < 0)
           -E_NO_MEM;
        e->env_ipc_perm = perm;
    }
    else
...
sys_page_clean_access(envid_t envid, void *va)
{
    ...
    page_clean_access(e->env_pgdir, va);
    return 0;
}
...
syscall(uint32_t syscallno, uint32_t a1, uint32_t a2, uint32_t a3, uint32_t a4, uint32_t a5)
{
    ...
    // Lab 5 Challenge
    case SYS_page_clean_access:
        return sys_page_clean_access((envid_t)a1, (void *)a2);
	default:
		return -E_INVAL;
	}
}
